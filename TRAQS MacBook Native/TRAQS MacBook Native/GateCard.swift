...
// The gate's two-position switch used to live here as `GateGlassToggle`. It has
// been replaced by `GlassSegmentedToggle` (its own file), which implements
// docs/LIQUID_GLASS_TOGGLE_LIFT_BRIEF.md — the old one had no
// matchedGeometryEffect, no GlassEffectContainer, no elevation track and no
// travel delay, so its thumb slid flat instead of lifting.
