              ...
              <div style={{ flex: 1, position: "relative", display: "flex" }}>
                ...
                {bars.map(bar => {
                  ...
                  return <div key={bar.id} title={bar.title + (bar.clientName ? ` (${bar.clientName})` : "") + (barLocked ? " 🔒 Locked" : "") + (hasMoveLog ? " 📋 Has schedule changes" : "") + (bar.hasSubs ? " (click to expand)" : "")}
                    ...
                    <span style={{ fontSize: 11, color: isPto ? bar.color : "#fff", fontWeight: 600, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", position: "relative", zIndex: 3, flex: 1 }}>{isPto ? `${bar.ptoType === "UTO" ? "📋" : "🏖️"} ${bar.title}` : (bar.task?.title || bar.title)}</span>
